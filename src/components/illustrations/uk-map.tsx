...
// Hand-plotted approximate positions on the 520x480 viewBox — coordinates
// only, never job counts. A city in the live data with no entry here is
// simply not plotted (still shown in the ranked list beside the map).
const CITY_COORDS: Record<string, { x: number; y: number }> = {
  London:     { x: 330, y: 388 },
  Manchester: { x: 255, y: 208 },
  Edinburgh:  { x: 260, y:  80 },
  Cambridge:  { x: 355, y: 318 },
  Bristol:    { x: 232, y: 364 },
  Oxford:     { x: 296, y: 350 },
  Leeds:      { x: 280, y: 220 },
  Birmingham: { x: 278, y: 298 },
  Glasgow:    { x: 240, y:  92 },
  Cardiff:    { x: 218, y: 360 },
  Liverpool:  { x: 224, y: 206 },
  Sheffield:  { x: 270, y: 236 },
  Nottingham: { x: 276, y: 268 },
  Belfast:    { x: 150, y: 130 },
  Newcastle:  { x: 288, y: 150 },
  // Common spelling variants that reach us un-normalised from scraped locations
  "Newcastle upon Tyne": { x: 288, y: 150 },
  "Newcastle Upon Tyne": { x: 288, y: 150 },
};

const CONNECTIONS: [string, string][] = [
  ["London", "Manchester"], ["London", "Cambridge"], ["London", "Oxford"], ["London", "Bristol"],
  ["Manchester", "Leeds"], ["Manchester", "Birmingham"], ["Manchester", "Edinburgh"],
  ["Edinburgh", "Glasgow"],
];
...
export function UKMap({
  ...
  // Plot only cities we have hand-placed coordinates for, sized by real
  // job_count relative to the largest value in the live set.
  ...
  const maxJobs = plotted[0]?.job_count ?? 1;
  const radiusFor = (jobs: number) => 7 + (jobs / maxJobs) * 20;
  const top = plotted[0];
  ...
  return (
    <div className={`relative select-none ${className}`}>
      ...
      >
        ...
        {/* Layer 4 — connections between major hubs (decorative, not data) */}
        {CONNECTIONS.map(([a, b]) => {
        ...
        {/* Layer 5 — hiring-density heat glow, radius driven by real job_count */}
        {plotted.map((c) => (
          <circle key={`heat-${c.city}`} cx={c.x} cy={c.y} r={radiusFor(c.job_count) * 2.4} fill="url(#heatGlow)" />
        ))}

        {/* Layer 6 — city dots, sized by real job_count */}
        {plotted.map((c) => {
          const isTop = c.city === top?.city;
          const r = radiusFor(c.job_count);
          return (
            <g key={c.city}>
              {isTop && animated && (
                <>
                  <circle cx={c.x} cy={c.y} r={r + 18} fill="#4F46E5" fillOpacity="0.12"
                    style={{ animation: "ukPulse 2.8s ease-out infinite" }} />
                  <circle cx={c.x} cy={c.y} r={r + 10} fill="none" stroke="#818CF8" strokeWidth="1" opacity="0.25"
                    style={{ animation: "ukPulse 2.8s ease-out 0.6s infinite" }} />
                </>
              )}
              <circle cx={c.x + 1} cy={c.y + 2} r={r} fill="#3730A3" opacity="0.18" />
              <circle cx={c.x} cy={c.y} r={r}
                fill={isTop ? "url(#lonFill)" : "url(#cityFill)"}
                filter={isTop ? "url(#dotGlowBig)" : "url(#dotGlow)"}
              />
              <circle cx={c.x - r * 0.28} cy={c.y - r * 0.28} r={r * 0.38} fill="white" opacity="0.4" />
              <circle cx={c.x - r * 0.1} cy={c.y - r * 0.1} r={r * 0.18} fill="white" opacity="0.7" />
              {isTop && (
                <circle cx={c.x} cy={c.y} r={r + 5} fill="none" stroke="#C7D2FE" strokeWidth="1.5"
                  strokeDasharray="8 4" opacity="0.55" />
              )}

              {/* Layer 7 — labels, real job counts */}
              {showLabels && (
                <g>
                  <rect
                    x={c.x + r + 5} y={c.y - (isTop ? 11 : 7)}
                    width={c.city.length * (isTop ? 6.5 : 5.8) + 10} height={isTop ? 24 : 16}
                    rx="5" fill="white" opacity="0.9"
                  />
                  <text x={c.x + r + 10} y={c.y + (isTop ? 1 : 3)}
                    fontSize={isTop ? "10" : "8.5"} fontFamily="ui-sans-serif,system-ui,sans-serif"
                    fontWeight={isTop ? "800" : "700"} fill={isTop ? "#3730A3" : "#4338CA"}>
                    {c.city}
                  </text>
                  {isTop && (
                    <text x={c.x + r + 10} y={c.y + 12} fontSize="7.5"
                      fontFamily="ui-sans-serif,system-ui,sans-serif" fill="#6366F1" fontWeight="600">
                      {c.job_count.toLocaleString()} roles
                    </text>
                  )}
                </g>
              )}
        ...
        <g transform="translate(14,370)">
          ...
          <text x="36" y="14" fontSize="8.5" fontWeight="800" fill="#1E1B4B"
            fontFamily="ui-sans-serif,system-ui,sans-serif">{top?.city ?? "—"}</text>
          <text x="36" y="24" fontSize="7" fill="#818CF8"
            fontFamily="ui-sans-serif,system-ui,sans-serif">{(top?.job_count ?? 0).toLocaleString()} AI roles · top hub</text>
          ...
          <text x="36" y="52" fontSize="7" fill="#818CF8"
            ...
            {plotted.length > 1 ? `${plotted[plotted.length - 1].job_count.toLocaleString()} – ${(plotted[1]?.job_count ?? 0).toLocaleString()} roles each` : "—"}
          </text>
          ...
          <circle cx="14" cy="81" r="3" fill={isLive ? "#059669" : "#94A3B8"} />
          <circle cx="14" cy="81" r="5" fill={isLive ? "#059669" : "#94A3B8"} opacity="0.2" />
          <text x="24" y="84.5" fontSize="7" fill={isLive ? "#059669" : "#94A3B8"} fontWeight="700"
